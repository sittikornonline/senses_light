...
void setBeacon()
{
  ...
  Serial.printf("Random volt is %d\n", volt); 
  
  float tempFloat = random(2000, 3100) / 100.0f;
  Serial.printf("Random temp is %.2fC\n", tempFloat); 
  Serial.println("---------------------");
  ...
  oScanResponseData.setServiceData(BLEUUID(beconUUID), std::string(beacon_data, 17));
  //oScanResponseData.setServiceData(BLEUUID(beconUUID), std::string("thailand"));
  oAdvertisementData.setName("TLMBeacon2");
