...
void f_Task_spiff( void * pvParameters ) {
  ...
  for (;;) {
    ...
    if (flag_spiff == true)
    {
      ...
      if (code_cmd_spiff == 1) {
      ...
      else if (code_cmd_spiff == 64) {
        //Read buffer
        int _cnt_payload = min_file_4;
        EEPROM.writeString(add_cnt_payload, String(_cnt_payload));
        EEPROM.commit();
        Serial.println("========== Read buffer  ==============");
        String sdFile = spiff.readFile(SPIFFS, "/buffer_4.txt");
        Serial.println("4 readFile buffer  : "); Serial.println(sdFile);
        cnt_payload = min_file_4;
      }



      else if (code_cmd_spiff == 65) {
        //Read buffer
        int _cnt_payload = min_file_5;
        EEPROM.writeString(add_cnt_payload, String(_cnt_payload));
        EEPROM.commit();
        Serial.println("========== Read buffer  ==============");
        String sdFile = spiff.readFile(SPIFFS, "/buffer_5.txt");
        Serial.println("5 readFile buffer  : "); Serial.println(sdFile);
        cnt_payload = min_file_5;
      }


      else if (code_cmd_spiff == 66) {
        //Read buffer
        int _cnt_payload = min_file_6;
        EEPROM.writeString(add_cnt_payload, String(_cnt_payload));
        EEPROM.commit();
        Serial.println("========== Read buffer  ==============");
        String sdFile = spiff.readFile(SPIFFS, "/buffer_6.txt");
        Serial.println("6 readFile buffer  : "); Serial.println(sdFile);
        cnt_payload = min_file_6;
      }
        ...
        Serial.print("delete cnt_payload       : "); Serial.println(cnt_payload);
